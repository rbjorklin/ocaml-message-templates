(** Advanced logging example - demonstrates file rolling, multiple sinks, and filtering *)
...
let () =
  ...
  let process_user_request user_id request_type =
    Log_context.with_property "RequestId" (`String ("req-" ^ string_of_int (Random.int 10000))) (fun () ->
  ...
  for i = 1 to 20 do
    ...
    let request_type = List.nth request_types (Random.int (List.length request_types)) in
