...
let temp_file () =
  Filename.temp_file "test_sink_" ".log"
...
let test_file_sink_basic () =
  ...
  check bool "File contains first message" true (contains "First message" content);
  check bool "File contains second message" true (contains "Second message" content);
...
let test_file_sink_rolling () =
  (* This is a basic test - we can't easily test actual rolling without
     waiting for time to pass, but we can test that the sink accepts
     the rolling parameter *)
  let path = temp_file () in
...
let test_composite_sink () =
  ...
  check bool "Sink 1 contains message" true (contains "Composite test message" content1);
  check bool "Sink 2 contains message" true (contains "Composite test message" content2);
...
let test_console_sink () =
  (* Console sink is hard to test automatically, but we can at least
...
let () =
  run "Sink Tests" [
    ...
    "file_sink", [
      ...
      test_case "File sink rolling configuration" `Quick test_file_sink_rolling;
    ];
    "composite_sink", [
      test_case "Composite sink routes to multiple sinks" `Quick test_composite_sink;
    ];
    "console_sink", [
      test_case "Console sink basic operations" `Quick test_console_sink;
      test_case "Console sink with colors" `Quick test_console_sink_with_colors;
    ];
  ]
