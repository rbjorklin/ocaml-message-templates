...
let test_enqueue_multiple () =
  ...
  for _i = 1 to 10 do
    let event =
      Log_event.create ~level:Level.Information ~message_template:"Message"
        ~rendered_message:"Message" ~properties:[] ()
    in
    Async_sink_queue.enqueue queue event
  done;
...
let test_enqueue_drops_when_full () =
  ...
  for _i = 1 to 5 do
    let event =
      Log_event.create ~level:Level.Information ~message_template:"Msg"
        ~rendered_message:"Msg" ~properties:[] ()
    in
    Async_sink_queue.enqueue queue event
  done;
...
let test_flush_empties_queue () =
  let emit_fn, events = create_mock_sink () in
  let config =
    {Async_sink_queue.default_config with flush_interval_ms= 100000}
  in
  let queue = Async_sink_queue.create config emit_fn in
  ...
  check int "Queue has 5 before flush" 5
    (Async_sink_queue.get_queue_depth queue);
...
let test_close_flushes () =
  ...
  check int "Queue has pending before close" 5
    (Async_sink_queue.get_queue_depth queue);
  Async_sink_queue.close queue;
...
let test_concurrent_access () =
  ...
  let final_stats = Async_sink_queue.get_stats queue in
  check int "All 100 enqueued accounted for" 100
