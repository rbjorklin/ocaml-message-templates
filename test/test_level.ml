...
let test_level_ordering () =
  ...
  check bool "Debug < Information" true
    (Level.compare Level.Debug Level.Information < 0);
  check bool "Information < Warning" true
    (Level.compare Level.Information Level.Warning < 0);
  ...
  check bool "Verbose = Verbose" true
    (Level.compare Level.Verbose Level.Verbose = 0)
;;
...
let test_level_of_string () =
  ...
  check_level_opt "Information" (Some Level.Information)
    (Level.of_string "Information");
...
let () =
  run "Level Tests"
    ...
    ; ( "comparison"
      , [ test_case "Level comparison operators" `Quick
            test_level_comparison_operators ] ) ]
;;
