...
let prop_level_comparison =
  Test.make
    ~gen:gen_level
    ~name:"Levels can be compared"
    ~print:Message_templates.Level.to_string
    (fun level ->
...
let prop_level_filter_safe =
  Test.make
    ~gen:gen_level
    ~name:"Any level can create a filter"
    ~print:Message_templates.Level.to_string
    (fun level ->
      try
...
let prop_double_negation =
  Test.make
    ~gen:gen_level
    ~name:"Filter double negation works"
    ~print:Message_templates.Level.to_string
    (fun level ->
      try
        let filter = Message_templates.Filter.by_level level in
        let double_not = Message_templates.Filter.not (Message_templates.Filter.not filter) in
        let event = Message_templates.Log_event.create
          ~level ~message_template:"test" ~rendered_message:"test" ~properties:[] ()
        in
...
let prop_empty_all_filter =
  Test.make
    ~gen:gen_level
    ~name:"Empty ALL filter matches all"
    ~print:Message_templates.Level.to_string
    (fun level ->
      try
        let event = Message_templates.Log_event.create
