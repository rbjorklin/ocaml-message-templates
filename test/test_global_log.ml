...
let temp_file () = Filename.temp_file "test_global_log_" ".log"
...
let test_log_context_push_pop () =
  ...
  check bool "RequestId still present" true
    (List.mem_assoc "RequestId" props_after_pop);
...
let test_log_context_with_property () =
  ...
  let result =
    Log_context.with_property "CorrelationId" (`String "corr-789") (fun () ->
        let props = Log_context.current_properties () in
  ...
  check bool "Property auto-popped" false
    (List.mem_assoc "CorrelationId" (Log_context.current_properties ()));
...
let test_log_context_with_exception () =
  ...
  with Failure _ ->
    ...
    check bool "Original property preserved" true
      (List.mem_assoc "TestProp" (Log_context.current_properties ()));

    (* Temp property should be popped even with exception *)
    check bool "Temp property popped" false
      (List.mem_assoc "TempProp" (Log_context.current_properties ()));
...
let test_log_context_with_scope () =
  ...
  let result =
    Log_context.with_scope (fun () ->
        ...
        List.mem_assoc "OuterProp" inner_props
        && List.mem_assoc "InnerProp" inner_props )
  in
  ...
  check bool "InnerProp removed by scope" false
    (List.mem_assoc "InnerProp" outer_props);
...
let test_log_context_with_properties () =
  ...
  let result =
    Log_context.with_properties props (fun () ->
  ...
  check bool "All properties popped" true
  ...
  run "Global Log Tests"
    [ ( "basic"
      , [ test_case "Log set_logger and get_logger" `Quick test_log_set_and_get
        ...
        ; test_case "Log handles no logger gracefully" `Quick
            test_log_no_logger_configured
    ...
    ; ( "context"
      , [ test_case "Context push and pop" `Quick test_log_context_push_pop
        ; test_case "Context with_property auto-pops" `Quick
            test_log_context_with_property
        ; test_case "Context handles exceptions" `Quick
            test_log_context_with_exception
