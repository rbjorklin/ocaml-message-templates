...
let temp_file () =
  Filename.temp_file "test_logger_" ".log"
...
let test_logger_is_enabled () =
  ...
  check bool "Debug disabled for Info logger" false (Logger.is_enabled info_logger Level.Debug);
  check bool "Information enabled for Info logger" true (Logger.is_enabled info_logger Level.Information);
  check bool "Warning enabled for Info logger" true (Logger.is_enabled info_logger Level.Warning);
  
  (* Error logger should have Warning disabled, Error enabled *)
  check bool "Warning disabled for Error logger" false (Logger.is_enabled error_logger Level.Warning);
  check bool "Error enabled for Error logger" true (Logger.is_enabled error_logger Level.Error);
  check bool "Fatal enabled for Error logger" true (Logger.is_enabled error_logger Level.Fatal);
...
let test_logger_multiple_sinks () =
  ...
  let logger = Logger.create ~min_level:Level.Information ~sinks:[sink1; sink2] in
...
let test_logger_enricher () =
  ...
  check bool "Enriched message logged" true (contains "Enriched message" content);
...
let () =
  run "Logger Tests" [
    "level_filtering", [
      test_case "Level filtering works correctly" `Quick test_logger_level_filtering;
      test_case "is_enabled returns correct values" `Quick test_logger_is_enabled;
    ];
    "context", [
      test_case "Context properties added to events" `Quick test_logger_context;
    ];
    "exceptions", [
      test_case "Exceptions can be logged" `Quick test_logger_write_with_exception;
    ];
    "sinks", [
      test_case "Multiple sinks receive events" `Quick test_logger_multiple_sinks;
    ];
    "filters", [
      test_case "Additional filters can be added" `Quick test_logger_add_min_level_filter;
    ];
    "enrichment", [
