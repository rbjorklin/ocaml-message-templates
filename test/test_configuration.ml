...
let temp_file () = Filename.temp_file "test_config_" ".log"
...
let test_filter_level_filter () =
  ...
  check bool "Warning < Information" true
    (Filter.level_filter Level.Information event)
;;
...
let test_filter_matching () =
  ...
  check bool "RequestId property exists" true
    (Filter.matching "RequestId" event);
  check bool "Other property does not exist" false
    (Filter.matching "OtherId" event)
;;
...
let test_filter_any () =
  ...
  let filter1 = Filter.level_filter Level.Error in
  (* This will fail *)
  let filter2 = Filter.matching "RequestId" in
  (* This will pass *)
...
let test_filter_not () =
  ...
  (* Debug < Information, so level_filter returns false, not_filter returns
     true *)
...
let () =
  run "Configuration Tests"
    [ ( "basic"
      , [ test_case "Configuration can be created" `Quick
            test_configuration_create
        ; test_case "Minimum level can be set" `Quick
            test_configuration_minimum_level
        ; test_case "Level convenience methods work" `Quick
            test_configuration_level_convenience_methods ] )
    ; ( "enrichment"
      , [ test_case "Properties can be added via configuration" `Quick
            test_configuration_enrich_with_property ] )
    ; ( "filtering"
      , [ test_case "Filter by minimum level works" `Quick
            test_configuration_filter_by_min_level
        ; test_case "Level filter predicates work" `Quick
            test_filter_level_filter
