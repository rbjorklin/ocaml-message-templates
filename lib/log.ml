...
let set_logger logger = global_logger := Some logger

(** Get the current global logger (if set) *)
let get_logger () = !global_logger
...
let debug ?exn message properties = write ?exn Level.Debug message properties
...
let error ?exn message properties = write ?exn Level.Error message properties

let fatal ?exn message properties = write ?exn Level.Fatal message properties
