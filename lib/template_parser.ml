...
(* Text content: any chars except '{' and sequences of single '}' not followed by '}' *)
let text =
  let rec text_parts acc =
    ...
    | Some '}' ->
        ...
        peek_string 2 >>= fun s ->
        if s = "}}" then
          (* This is an escaped brace - stop here so escaped_brace can handle it *)
