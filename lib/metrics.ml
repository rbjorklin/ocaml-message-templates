...
let update_percentiles latencies =
  if Queue.is_empty latencies then
    ...
    let arr =
      Queue.fold (fun acc x -> x :: acc) [] latencies |> Array.of_list
    in
...
(** Get or create sink data - internal use only, must hold lock *)
...
let to_json t =
  ...
  let json_metrics =
    List.map
      (fun (metrics : sink_metrics) ->
        let error_json =
          match metrics.last_error with
          | None -> `Null
          | Some (exn, timestamp) ->
              `Assoc
  ...
  in
  `Assoc
    [("timestamp", `Float (Unix.gettimeofday ())); ("sinks", `List json_metrics)]
;;
