...
let expand_log_level level_name ~ctxt template_str =
  ...
  let properties = 
    List.map (fun (hole : Message_templates.Types.hole) ->
      ...
      Ast_builder.Default.pexp_tuple ~loc [
        ...
        [%expr `String (Message_templates.Runtime_helpers.to_string [%e var_expr])]
      ]
    ) holes
  in
...
let () =
  let extensions = List.map (fun (level, _) ->
    let ext = create_level_extension level in
    Ppxlib.Context_free.Rule.extension ext
  ) level_functions in
  
  Driver.register_transformation ~rules:extensions "message-templates-log-levels"
