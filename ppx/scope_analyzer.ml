...
let push_scope scope =
  { bindings = []; outer_scopes = scope :: scope.outer_scopes }
...
let rec extract_pattern_names pat =
  ...
  | Ppat_record (fields, _) ->
      List.map (fun (_, pat) -> extract_pattern_names pat) fields
      |> List.concat
...
let scope_from_params pat =
  ...
  List.fold_left (fun scope name -> add_binding name None scope) empty_scope names
