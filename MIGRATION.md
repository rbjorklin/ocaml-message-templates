...
try risky_operation ()
with exn ->
  Log.error "Operation failed" [("op", `String "risky")]
```
...
```ocaml
let json_sink_instance = Json_sink.create "output.json" in
let json_sink_config =
  Configuration.sink_config
    { Composite_sink.emit_fn = (fun e -> Json_sink.emit json_sink_instance e)
...
| Async logging | No | No | Yes (Lwt/Eio) |
...
| Old API | New API |
|---------|---------|
| `Configuration.create_logger` | `Configuration.create_logger` |
...
Configuration.create () |> Configuration.write_to_console ~colors () |> Configuration.create_logger
```
